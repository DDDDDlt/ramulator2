...
def plot_metrics(normalized_data, output_prefix='auto_hw_metrics'):
    """
    Plot latency and energy comparison
    """
    # 模型名称（5个模型）+ 平均值
    models = [
    ...
    acc_colors = {
        'FlexPosit': '#4FB0A9',   # teal primary color (consistent with hardware figure)
        'BitMod': '#F4A261',     # warm orange (bit-serial path)
        'Olive': '#457B9D',      # deep teal blue (contrast)
        'Baseline': '#BDBDBD'    # light gray (baseline)
    }
    ...
    # (1) Latency plot
    for j, acc in enumerate(accelerators):
        x = x_base + (j - len(accelerators)//2) * bar_width * bar_spacing  # add intra-group spacing
        ...
        for i, (bar, val) in enumerate(zip(bars, norm_cycle[:, j])):
            if val > 0.05:  # show labels only for sufficiently large values
    ...
    axes[0].set_ylim(0, max(1.1, max_latency * 1.15))  # leave room for legend
    ...
    labels[-1].set_color('#4FB0A9')  # highlight Average tick (teal)
    ...
    axes[1].set_ylim(0, max(1.15, max_energy * 1.15))  # leave room for legend
    ...
    # X label
    axes[1].set_xlabel("(b) Energy Consumption Comparison", fontweight='bold', fontsize=12, labelpad=10)
    ...
    for ax in axes:
        ...
        avg_pos = x_base[-1]  # position of Average
        ...
        for i in range(1, len(models) - 1):  # up to the second last
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Auto plot metrics from logs with selectable config')
    ...
    print("="*60)
    print("🚀 Start reading logs and generating plots")
    print("="*60)
